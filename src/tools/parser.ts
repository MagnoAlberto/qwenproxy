...
export class StreamingToolParser {
    ...
    while (this.buffer.length > 0) {
      if (!this.insideTool) {
        ...
        if (match && match.index !== undefined) {
          // Text before the tool call tag – emit immediately
    ...
    if (this.insideTool) {
      ...
      if (trimmed.length > 0) {
        ...
        if (recovered) {
          ...
          // Recovery failed – push entire block back as text with tags
  ...
  private processToolContent(content: string, result: ParserResult): void {
    ...
    if (!t) {
      // Empty tool call - malformed. Restore original tags as text.
    ...
    // Restore original content with tags as text so callers can inspect it.
